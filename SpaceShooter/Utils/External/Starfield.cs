...
namespace SpaceShooter.Utils.External {
	...
	/// </summary>
	internal class Starfield : IDisposable {
		#region Constants
		...
		/// </summary>
		static readonly float[] MovementFactors = {
		...
		public void LoadContent() {
			...
			cloudTexture = contentManager.Load<Texture2D>("Effects/cloudTexture");

			// load the cloud effect
			cloudEffect = contentManager.Load<Effect>("Effects/cloudEffect");
		...
		public void Reset(Vector2 position) {
			...
			for(int i = 0; i < stars.Length; ++i) {
				stars[i] = new Vector2(Assets.Random.Next(0, viewportWidth),
					Assets.Random.Next(0, viewportHeight));
			}
		...
		public void Draw(Vector2 position) {
			...
			for(int i = 0; i < stars.Length; i++) {
				...
				if(stars[i].X < starfieldRectangle.X) {
					...
					stars[i].Y = starfieldRectangle.Y + Assets.Random.Next(starfieldRectangle.Height);
				}
				...
				if(stars[i].Y < starfieldRectangle.Y) {
					stars[i].X = starfieldRectangle.X + Assets.Random.Next(starfieldRectangle.Width);
